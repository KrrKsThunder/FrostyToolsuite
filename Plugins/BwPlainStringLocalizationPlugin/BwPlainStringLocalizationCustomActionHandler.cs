...
{
    public class BwPlainStringLocalizationCustomActionHandler: ICustomActionHandler
    {

        // The name for option to show all text ids, this can be set with the bioware localization plugin - i currently see no reason to include a different setting with the same meaning for this plugin.
        ...
        private class BwPlainModResource: EditorModResource
        {
            ...
            public BwPlainModResource(ResAssetEntry inEntry, FrostyModWriter.Manifest inManifest) : base(inEntry)
            {

                // This constructor does the exact same thing as the ones in the TestPlugin

                // obtain the modified data
                ModifiedPlainLocalizationResource md = inEntry.ModifiedEntry.DataObject as ModifiedPlainLocalizationResource;
                byte[] data = md.Save();
        ...
        {
            writer.AddResource(new BwPlainModResource(entry as ResAssetEntry, writer.ResourceManifest));
        }
        #endregion
        ...
        public IEnumerable<string> GetResourceActions(string name, byte[] data)
        {
            ...
            if( !Config.Get(SHOW_INDIVIDUAL_TEXTIDS_OPTION_NAME, false, ConfigScope.Global))
            {
        ...
        public object Load(object existing, byte[] newData)
        {
            ...
            ModifiedPlainLocalizationResource edited = (ModifiedPlainLocalizationResource)existing;
            ModifiedPlainLocalizationResource newTexts = (ModifiedPlainLocalizationResource) ModifiedResource.Read(newData);
        ...
        public void Modify(AssetEntry origEntry, AssetManager am, RuntimeResources runtimeResources, object data, out byte[] outData)
        {
            ...
            ModifiedPlainLocalizationResource modified = data as ModifiedPlainLocalizationResource;
            ...
            if(resource == null)
            {
                throw new ArgumentNullException("resource", string.Format("Resource in BwPlainStringLocalizationHandler Modify(...) is null after GetResAs call for <{0}>!", origEntry.Name));
            }
